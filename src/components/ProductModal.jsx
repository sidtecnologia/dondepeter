...
export default function ProductModal({ product, onClose, onAdd }) {
  ...
  return (
    <div id="productModal" className="modal" style={{ display: 'flex' }} aria-hidden="false">
      <div className="modal-content-wrapper">
        <div className="modal-card">
          ...
          <div className="carousel-container" aria-live="polite">
            <button id="prev-btn" className="carousel-btn prev-btn" aria-label="Anterior" onClick={() => {
            ...
            }}><i className="fas fa-circle-arrow-left"></i></button>
            <div id="carousel-images-container" className="carousel-images-container" role="list">
              {images.length === 0 ? <div className="carousel-image" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f3f3f3' }}>Sin imagen</div> : images.map((src, idx) => <img key={idx} src={src} className="carousel-image" loading="lazy" style={{ display: idx === 0 ? '' : 'none' }} />)}
            </div>
            <button id="next-btn" className="carousel-btn next-btn" aria-label="Siguiente" onClick={() => {
              const el = document.getElementById('carousel-images-container')
              if (!el) return
              const imgs = el.querySelectorAll('.carousel-image')
              if (imgs.length === 0) return
              ...
              imgs.forEach((im, i) => im.style.display = i === idx ? '' : 'none')
            }}><i className="fas fa-circle-arrow-right"></i></button>
          </div>
